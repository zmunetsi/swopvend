...
export default function  ItemUploadPage() {
  ...
  const [form, setForm] = useState({
    username: 'jane_doe', // fetched from API
    first_name: '',
    last_name: '',
    email: 'jane@example.com', // fetched from API
    city: '',
    postcode: '',
    country: { name: '', code: '' }, // fetched from API
    phone: '',
    profile_image: null
  });
    ...
    apiClient.get('/traders/me/')
      .then(({ data }) => {
        ...
        setForm({
          ...
          phone: data.phone || '',
          profile_image: null         // we’ll handle uploads separately
        });
      })
      .catch((err) => {
        console.error(err);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiClient.patch('/traders/me/', payload, {
      ...
      <div className="flex flex-column flex-auto">
        <div>
          ...
          <div className="font-medium mb-4">
            Welcome back, <span className="font-bold">{form.first_name || form.username}!</span> 👋
          </div>
